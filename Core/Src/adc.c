...
void pa1_adc_dma_init(uint32_t src, uint32_t dst)
{
	...
	// Set mode of PA1 to analog mode, reference manual p186 11-analog mode
	GPIOA->MODER |= (1U<<2);
	GPIOA->MODER |= (1U<<3);
	...
	ADC1->SQR3 = ADC_CH1; // Only one channel, ch 1. Reference manual p387,388. Sets up channel one as first (and only) sequence
	...
	/* Setting up ADC triggering from TIM2 CH2 (in the middle of the pulse - CNT=ARR) */

	// Enabling external trigger for channels
	ADC1->CR2 |=  CR2_EXTEN; // Reference manual p382, bits 29,28: 11 is trigger on both rising and falling edge of the trigger signal, this is chosen because output compare CH2 is in TOGGLE mode

	// Disabling continuous mode since triggering comes through TIM2 CH2
	ADC1->CR2 &= ~ CR2_CONT;

	// Selecting external event for trigger, TIM2 CH2
	ADC1->CR2 |= CR2_EXTSEL;
