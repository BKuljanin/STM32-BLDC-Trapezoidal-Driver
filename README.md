...
**Note:**
This project implements a complete brushless DC (BLDC) motor controller using **trapezoidal (six-step) commutation** on the STM32F446RE. All peripheral drivers are **register-level** (no HAL for TIM, ADC, I2C, GPIO). The firmware supports two commutation modes: **encoder-based closed-loop** control using the AS5600 magnetic encoder over I2C, and **sensorless back-EMF (BEMF)** zero-crossing detection. In encoder mode, a **PI speed controller** with anti-windup drives a smooth speed ramp to a configurable setpoint.
...
- **Six-step trapezoidal commutation** with phase enable/disable via GPIO
- **ADC1 injected channel** triggered at the center of the PWM pulse (ARR trigger via TIM1_CH4) for stable back-EMF sampling
- **Back-EMF zero-crossing detection** for sensorless commutation with 500 µs blanking
- **AS5600 14-bit magnetic encoder** via register-level I2C1 at 1 kHz (TIM3 interrupt-driven)
- Encoder **angle unwrapping** and **low-pass filtering** (30 Hz) to remove quantization noise
- Angular **speed estimation** from filtered encoder angle deltas with 20 Hz LP filter
- **PI speed controller** with integral windup protection (back-calculation clamping)
- **Speed ramp generator** with configurable acceleration rate (deg/s²)
- **Open-loop startup** sequence for BEMF mode: 300-step ramp from 20 ms to 2 ms per step
- **TIM2 1 MHz free-running counter** for microsecond-precise commutation scheduling
...
- Motor **angular position** in **deg** (absolute, unwrapped, filtered — AS5600)
- Motor **angular speed** in **deg/s** (filtered, updated at 1 kHz)
- **PI controller duty cycle output** in **%** (0–50%)
- **Back-EMF voltage** of the floating phase in **V** (converted from 12-bit ADC, scaled through IHM07 divider)
- **Commutation step** (0–5) indicating current electrical sector
...
- Divider ratio: (10 + 2.2) / 2.2 ≈ **5.55** — scales floating phase voltage (0–12 V) to ADC input range (0–3.3 V)
- Phase enable pins (active-high) control each gate driver independently

### Motor: A2208/14T 1400 KV Brushless Outrunner
- **Pole pairs:** 7
- Electrical angle = mechanical angle × 7
- 60° mechanical rotation = 420° electrical

### Encoder: AS5600 (14-bit Magnetic, I2C)
- 12-bit output resolution: 4096 counts/revolution
- Raw angle → degrees: `raw × 360 / 4096`
- I2C address: **0x36**
...
| GPIO_BEMF| PC9  | GPIO Output   | BEMF detection circuit enable (active-low) |
...
| Max duty cycle     | 50%      | Hard-clamped in `tim1_pwm_set_duty_percent()`       |
| CCR formula        | `ARR − (duty × ARR / 100)` | Inverted for PWM1 mode          |
| ADC trigger        | TIM1_CH4 | Toggle mode — fires ADC at ARR (pulse center)       |
| Preload            | Enabled  | OC1PE, OC2PE, OC3PE prevent mid-cycle glitches      |
| MOE                | Enabled  | BDTR bit 15                                         |

Center-aligned mode ensures the ADC is triggered exactly at the midpoint of the PWM pulse, where inductor current is flat and measurement noise is minimized.
...
- **Float phase:** Disable EN pin (both high and low MOSFETs OFF)
...
A **500 µs blanking period** suppresses false crossings from switching transients immediately after commutation. Duty cycle is fixed (20% by default in BEMF mode).

**Open-loop startup** (300 steps) spins the motor to a speed where BEMF amplitude is large enough to detect before handing off to sensorless tracking.
...
| Parameter        | Value           | Notes                                           |
|-----------------:|----------------:|-------------------------------------------------|
| Resolution       | 12-bit          | 4095 counts full-scale                          |
...
| ISR              | ADC_IRQHandler  | JEOC flag — reads JDR1, converts, stores        |
...
| Unwrapping         | ±180° jump detection | Maintains continuity through 360°→0° wraps |
| Speed units        | deg/s       |                                               |
| Direction          | Configurable| `BLDC_REVERSE` flips to `360 − angle`        |
...
integral  += error × dt
i_term     = integral × Ki          // clamped to ±integral_sat
output     = Kp × error + i_term    // clamped to [0, 50]%
```

If `i_term` exceeds `±integral_sat`, the integral accumulator is wound back proportionally (back-calculation anti-windup). The output is then hard-clamped to the duty cycle range.

**Speed ramp:** A ramp generator limits the setpoint rate of change to 100 deg/s² so that sudden speed commands do not demand a step current that exceeds the hardware limits or causes the PI output to wind up.

---

## Timing Architecture

| Timer | Clock | Configuration        | Purpose                                         |
|------:|------:|---------------------:|-------------------------------------------------|
| TIM1  | 100 MHz | 20 kHz center-aligned | Three-phase PWM + ADC trigger (CH4 toggle)  |
| TIM2  | 50 MHz | Prescaler=49, ARR=0xFFFFFFFF | 1 MHz free-running microsecond counter  |
| TIM3  | 50 MHz | Prescaler=49, ARR=999 | 1 kHz interrupt → encoder I2C trigger         |

TIM2 provides the timestamp for all BEMF commutation scheduling (`read_time_us()`) and the `dt` measurement for the PI controller. It wraps after ~4294 seconds.
...
9. **Motor parking:** Commutate to step 0 at 15% duty, wait 5000 ms for rotor to settle
10. **Reference zeroing** (encoder mode): Read first angle, call `as5600_set_reference()`, wait 2000 ms
11. Step = 1 → enter main control loop
...
```
TIM3 IRQ (1 kHz)
  └─ as5600_trigger_read()        // start non-blocking I2C read
...
Main loop (continuous)
  └─ bldc_run(20, BEMF_MODE)
       ├─ [blanked for 500 µs after commutation]
...
This maps the 0–12 V floating phase voltage into the 0–3.3 V ADC input range. A zero-crossing of the back-EMF (at VBUS/2 = 6 V) maps to an ADC code of ≈ 2048. After applying the conversion formula, `floating_phase_back_emf` crosses zero at the same moment as the actual BEMF zero-crossing, making sign-change detection straightforward.
...
- `bldc_run()` : Top-level run function, dispatches to encoder or BEMF logic
- `bldc_update_step()` : Converts encoder angle to electrical sector and commutation step
- `bldc_open_loop_run()` : 300-step open-loop ramp for BEMF startup
...
- `ADC_IRQHandler()` : JEOC ISR — reads JDR1, converts, updates `floating_phase_back_emf`

---

### `as5600.h / as5600.c`
AS5600 14-bit magnetic encoder driver.
- `as5600_init()` : Verifies device presence, reads initial angle
- `as5600_trigger_read()` : Starts non-blocking I2C burst read of angle registers (0x0C–0x0D)
- `as5600_process_reading()` : Converts raw count to degrees, unwraps, applies 30 Hz LP filter
- `as5600_calculate_speed()` : Computes angular speed from filtered angle delta, applies 20 Hz LP filter
...
- `tim2_1mhz_init()` : Configures TIM2 as 1 MHz free-running microsecond counter
...
- `I2C1_read_bytes()` : Non-blocking multi-byte read
...
| Back-EMF ADC sampling  | 40 kHz  | TIM1_CH4 toggle → ADC injected (both edges) |
| Encoder angle read     | 1 kHz   | TIM3 interrupt → non-blocking I2C   |
| Speed calculation      | 1 kHz   | On every encoder reading            |
| PI controller update   | 1 kHz   | On every speed calculation          |
| BEMF zero-cross check  | 40 kHz  | Continuous in main loop after each ADC ISR |
...
- IHM07M1 Schematic / User Manual — ST Motor Control Shield
